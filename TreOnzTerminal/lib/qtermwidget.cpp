...
Session *TermWidgetImpl::createSession(QWidget* parent)
{
    ...
    session->setCodec(QStringConverter::Encoding::Utf8);
...
void QTermWidget::search(bool forwards, bool next)
{
    ...
    */
    ...
    QString pattern = m_searchBar->searchText();

    // 2. If it's NOT a regex (FixedString), we must escape it
    if (!m_searchBar->useRegularExpression()) {
        pattern = QRegularExpression::escape(pattern);
    }

    // 3. Create the regex with the correct CaseSensitivity
...
void QTermWidget::setTextCodec(QStringConverter::Encoding encoding)
{
    if (!m_impl->m_session)
        return;
