...
QList<KeyboardTranslatorReader::Token> KeyboardTranslatorReader::tokenize(const QString& line)
{
    ...
    //static QRegularExpression title(u"keyboard\\s+\"(.*)\""_s);
    static QRegularExpression title(QStringLiteral("keyboard\\s+\"(.*)\""));
    ...
    if ( text.isEmpty() )
    {
     ...
     QRegularExpressionMatch match1 = title.match(text);
     if (match1.hasMatch() && match1.capturedLength() == text.length())
    {
        Token titleToken = { Token::TitleKeyword , QString() };
        Token textToken = { Token::TitleText , match1.capturedTexts().at(1) };

        list << titleToken << textToken;
    }
    else
     {
         QRegularExpressionMatch match2 = key.match(text);
        if ( match2.hasMatch() && match2.capturedLength() == text.length() )
        {
            Token keyToken = { Token::KeyKeyword , QString() };
            Token sequenceToken = { Token::KeySequence , match2.capturedTexts().value(1).remove(QLatin1Char(' ')) };

            list << keyToken << sequenceToken;

            if ( match2.capturedTexts().at(3).isEmpty() )
            {
            // capturedTexts()[2] is a command
            Token commandToken = { Token::Command , match2.capturedTexts().at(2) };
            list << commandToken;
            }
            else
            {
            // capturedTexts()[3] is the output string
            Token outputToken = { Token::OutputText , match2.capturedTexts().at(3) };
            list << outputToken;
            }
        }
        else
...
QByteArray KeyboardTranslator::Entry::escapedText(bool expandWildCards,Qt::KeyboardModifiers modifiers) const
{
    ...
    for ( int i = 0 ; i < result.count() ; i++ )
    {
        ...
        if ( replacement == 'x' )
        {
            QByteArray hexPart = QByteArray(1, ch).toHex();
            QByteArray replacement = "\\x";
            replacement.append(hexPart);
