...
//     if (match.hasMatch() && match.capturedLength() == emptyString.length())
//         return;
...
//         QRegularExpressionMatch match = _searchText.match(*text, pos);
//         if (match.hasMatch()) {
...
void RegExpFilter::process()
{
    ...
    // Fixed exactMatch replacement from earlier
    QRegularExpressionMatch emptyMatch = _searchText.match(emptyString);
    if (emptyMatch.hasMatch() && emptyMatch.capturedLength() == emptyString.length())
        return;

    while(pos >= 0)
    {
        // In Qt 6, we must capture the match result to get start/length/texts
        QRegularExpressionMatch match = _searchText.match(*text, pos);

        if (match.hasMatch())
        {
            pos = match.capturedStart(); // Replacement for indexIn()
            ...
            int matchLen = match.capturedLength(); // Replacement for matchedLength()

            getLineColumn(pos, startLine, startColumn);
            getLineColumn(pos + matchLen, endLine, endColumn);

            RegExpFilter::HotSpot* spot = newHotSpot(startLine, startColumn,
                                                   endLine, endColumn);

            // Replacement for _searchText.capturedTexts()
...
UrlFilter::HotSpot::UrlType UrlFilter::HotSpot::urlType() const
{
    ...
    // Match Full URL
    QRegularExpressionMatch urlMatch = FullUrlRegExp.match(url);
    if (urlMatch.hasMatch() && urlMatch.capturedLength() == url.length())
        return StandardUrl;

    // Match Email
    QRegularExpressionMatch emailMatch = EmailAddressRegExp.match(url);
    if (emailMatch.hasMatch() && emailMatch.capturedLength() == url.length())
        return Email;
